"use strict";
var calendarHeatmap = {settings:
            {gutter: 5, item_gutter: 1, width: 1e3, height: 200, item_size: 10, label_padding: 40, max_block_height: 20, transition_duration: 500, tooltip_width: 250, tooltip_padding: 15}, init: function (a, e, t, n) {
        calendarHeatmap.data = a, calendarHeatmap.color = e || "#ff4500", calendarHeatmap.overview = t || "global", calendarHeatmap.history = ["global"], calendarHeatmap.selected = {}, calendarHeatmap.handler = n, calendarHeatmap.in_transition = !1, calendarHeatmap.createElements(), calendarHeatmap.parseData(), calendarHeatmap.drawChart()
    }, createElements: function () {
        var a = document.createElement("div");
        a.className = "calendar-heatmap", document.body.appendChild(a);
        var e = d3.select(a).append("svg").attr("class", "svg");
        calendarHeatmap.items = e.append("g"), calendarHeatmap.labels = e.append("g"),
        ...
        var t = function ()
        {
            var t = Math.round((moment() - moment().subtract(1, "year").startOf("week")) / 864e5), n = Math.trunc(t / 7) + 1;
        ...
        calendarHeatmap.data && (calendarHeatmap.data[0].summary || calendarHeatmap.data.map(function (a) {
            var e = a.details.reduce(function (a, e) {
                return a[e.name] ? a[e.name].value += e.value : a[e.name] = {value: e.value}, a
        ...
        var a = moment(calendarHeatmap.data[0].date).startOf("year"), e = moment(calendarHeatmap.data[calendarHeatmap.data.length - 1].date).endOf("year"), t = d3.timeYears(a, e).map(function (a) {
            var e = moment(a);
            return{date: e, total: calendarHeatmap.data.reduce(function (a, t) {
                    ...
                    var a = calendarHeatmap.data.reduce(function (a, t) {
                        if (moment(t.date).year() === e.year())
                            for (var n = 0; n < t.summary.length; n++)
                                a[t.summary[n].name] ? a[t.summary[n].name].value += t.summary[n].value : a[t.summary[n].name] = {value: t.summary[n].value};
                        return a
                    }, {});
        ...
        calendarHeatmap.items.selectAll(".item-block-year").data(t).enter().append("rect").attr("class", "item item-block-year").attr("width", function () {
            ...
            return d3.scaleLinear().range(["#ffffff", calendarHeatmap.color || "#ff4500"]).domain([-.15 * n, n])(a.total) || "#ff4500"
        }).on("click", function (a) {
            ...
            if (!calendarHeatmap.in_transition) {
                ...
                if (l > 0 && (e += n > 0 || r > 0 ? "<div><span></span><span>" + (1 === l ? "1 minute" : l + " minutes") + "</span></div>" : "<span>" + (1 === l ? "1 minute" : l + " minutes") + "</span></div>"), e += "<br />", a.summary.length <= 5)
                    for (o = 0; o < a.summary.length; o++)
                        e += "<div><span><strong>" + a.summary[o].name + "</strong></span>", e += "<span>" + calendarHeatmap.formatTime(a.summary[o].value) + "</span></div>";
                else {
                    for (o = 0; o < 5; o++)
                        e += "<div><span><strong>" + a.summary[o].name + "</strong></span>", e += "<span>" + calendarHeatmap.formatTime(a.summary[o].value) + "</span></div>";
                    e += "<br />";
                    for (var d = 0, o = 5; o < a.summary.length; o++)
                        d = +a.summary[o].value;
                    e += "<div><span><strong>Other:</strong></span>", e += "<span>" + calendarHeatmap.formatTime(d) + "</span></div>"
                }
                ...
                calendarHeatmap.tooltip.html(e).style("left", s + "px").style("top", m + "px").transition().duration(calendarHeatmap.settings.transition_duration / 2).ease(d3.easeLinear).style("opacity", 1)
            }
        ...
        calendarHeatmap.history[calendarHeatmap.history.length - 1] !== calendarHeatmap.overview && calendarHeatmap.history.push(calendarHeatmap.overview);
        var a = moment(calendarHeatmap.selected.date).startOf("year"), e = moment(calendarHeatmap.selected.date).endOf("year"), t = calendarHeatmap.data.filter(function (t) {
            return a <= moment(t.date) && moment(t.date) < e
        }), n = d3.max(t, function (a) {
            return a.total
        }), r = d3.scaleLinear().range(["#ffffff", calendarHeatmap.color || "#ff4500"]).domain([-.15 * n, n]), i = function (e) {
            var t = moment(e.date), n = Math.round((t - moment(a).startOf("week")) / 864e5);
            return Math.trunc(n / 7) * (calendarHeatmap.settings.item_size + calendarHeatmap.settings.gutter) + calendarHeatmap.settings.label_padding
        }, l = function (a) {
        ...
        calendarHeatmap.items.selectAll(".item-circle").remove(), calendarHeatmap.items.selectAll(".item-circle").data(t).enter().append("rect").attr("class", "item item-circle").style("opacity", 0).attr("x", function (a) {
            ...
            if (!calendarHeatmap.in_transition) {
                ...
                for (var n = 0; n < a.summary.length; n++)
                    t += "<div><span><strong>" + a.summary[n].name + "</strong></span>", t += "<span>" + calendarHeatmap.formatTime(a.summary[n].value) + "</span></div>";
        ...
        var o = d3.timeMonths(a, e), s = d3.scaleLinear().range([0, calendarHeatmap.settings.width]).domain([0, o.length]);
        calendarHeatmap.labels.selectAll(".label-month").remove(), calendarHeatmap.labels.selectAll(".label-month").data(o).enter().append("text").attr("class", "label label-month").attr("font-size", function () {
            ...
            calendarHeatmap.in_transition || calendarHeatmap.data.filter(function (e) {
                return moment(a).startOf("month") <= moment(e.date) && moment(e.date) < moment(a).endOf("month")
            }).length && (calendarHeatmap.selected = {date: a}, calendarHeatmap.in_transition = !0, calendarHeatmap.hideTooltip(), calendarHeatmap.removeYearOverview(), calendarHeatmap.overview = "month", calendarHeatmap.drawChart())
        });
        var m = d3.timeDays(moment().startOf("week"), moment().endOf("week")), c = d3.scaleBand().rangeRound([calendarHeatmap.settings.label_padding, calendarHeatmap.settings.height]).domain(m.map(function (a) {
            return moment(a).weekday()
        }));
        calendarHeatmap.labels.selectAll(".label-day").remove(), calendarHeatmap.labels.selectAll(".label-day").data(m).enter().append("text").attr("class", "label label-day").attr("x", calendarHeatmap.settings.label_padding / 3).attr("y", function (a, e) {
            return c(e) + c.bandwidth() / 1.75
        ...
        }), s = (calendarHeatmap.settings.width - calendarHeatmap.settings.label_padding) / l.length - 5 * calendarHeatmap.settings.gutter, m = d3.scaleLinear().rangeRound([0, s]);
        o.selectAll(".item-block-rect").data(function (a) {
            return a.summary
        }).enter().append("rect").attr("class", "item item-block-rect").attr("x", function (a) {
            var e = parseInt(d3.select(this.parentNode).attr("total")), t = parseInt(d3.select(this.parentNode).attr("offset"));
            return m.domain([0, e]), d3.select(this.parentNode).attr("offset", t + m(a.value)), t
        }).attr("width", function (a) {
            var e = parseInt(d3.select(this.parentNode).attr("total"));
            return m.domain([0, e]), Math.max(m(a.value) - calendarHeatmap.settings.item_gutter, 1)
        }).attr("height", function () {
            return Math.min(i.bandwidth(), calendarHeatmap.settings.max_block_height)
        }).attr("fill", function (a) {
            return d3.scaleLinear().range(["#ffffff", calendarHeatmap.color || "#ff4500"]).domain([-.15 * n, n])(a.value) || "#ff4500"
        }).style("opacity", 0).on("mouseover", function (a) {
            if (!calendarHeatmap.in_transition) {
                var e = new Date(d3.select(this.parentNode).attr("date")), t = "";
                ...
                for (var n = d(moment(e).week()) + calendarHeatmap.settings.tooltip_padding; calendarHeatmap.settings.width - n < calendarHeatmap.settings.tooltip_width + 3 * calendarHeatmap.settings.tooltip_padding; )
                    n -= 10;
            ...
            }).length && (calendarHeatmap.in_transition = !0, calendarHeatmap.selected = {date: a}, calendarHeatmap.hideTooltip(), calendarHeatmap.removeMonthOverview(), calendarHeatmap.overview = "week", calendarHeatmap.drawChart())
        ...
        calendarHeatmap.history[calendarHeatmap.history.length - 1] !== calendarHeatmap.overview && calendarHeatmap.history.push(calendarHeatmap.overview);
        var a = moment(calendarHeatmap.selected.date).startOf("week"), e = moment(calendarHeatmap.selected.date).endOf("week"), t = calendarHeatmap.data.filter(function (t) {
            return a <= moment(t.date) && moment(t.date) < e
        }), n = d3.max(t, function (a) {
            return d3.max(a.summary, function (a) {
                return a.value
            ...
            return moment(a).weekday()
        })), l = [a], d = d3.scaleBand().rangeRound([calendarHeatmap.settings.label_padding, calendarHeatmap.settings.width]).padding([.01]).domain(l.map(function (a) {
            return a.week()
        }));
        ...
        o.selectAll(".item-block-rect").data(function (a) {
            ...
            if (!calendarHeatmap.in_transition) {
                ...
                calendarHeatmap.tooltip.html(t).style("left", r + "px").style("top", l + "px").transition().duration(calendarHeatmap.settings.transition_duration / 2).ease(d3.easeLinear).style("opacity", 1)
            }
        }).on("mouseout", function () {
            calendarHeatmap.in_transition || calendarHeatmap.hideTooltip()
        }).transition().delay(function () {
        ...
        calendarHeatmap.items.selectAll(".item-block-year").transition().
                duration(calendarHeatmap.settings.transition_duration).ease(d3.easeLinear).style
                ("opacity", 0).remove(), calendarHeatmap.labels.selectAll(".label-year").remove()
    }, removeYearOverview: function () {
        calendarHeatmap.items.selectAll(".item-circle").transition().duration(
                calendarHeatmap.settings.transition_duration).ease(d3.easeLinear).style(
                "opacity", 0).remove(), calendarHeatmap.labels.selectAll(".label-day").remove(), calendarHeatmap.labels.selectAll(
                ".label-month").remove(), calendarHeatmap.hideBackButton()
    }, removeMonthOverview: function () {
        calendarHeatmap.items.selectAll(".item-block-month").selectAll(
                ".item-block-rect").transition().duration(calendarHeatmap.settings.transition_duration).ease(d3.easeLinear).style("opacity", 0).attr("x", function (a, e) {
            return e % 2 == 0 ? -calendarHeatmap.settings.width / 3 : calendarHeatmap.settings.width / 3
        }).remove(), calendarHeatmap.labels.selectAll(
                ".label-day").remove(), calendarHeatmap.labels.selectAll(
                ".label-week").remove(), calendarHeatmap.hideBackButton()
